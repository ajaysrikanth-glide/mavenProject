...
public class Organisations_Guidance_GetACallBack extends BaseTest{
	...
	ConfigReader config=new ConfigReader();
	 ...
	 public void org_Guidance_GetACallBack(String name, String phone, String company, String message, String remarks) throws InterruptedException {
		 	 
		 ExtentTest test=getExtentTest();
		 
		 organisations=new ForOrganisations(driver);
		 	 
		 boolean status=organisations.verifyGuidance();
		 
		 organisations.printHeaders();
		 
		 if(status==true) {
				
				test.log(LogStatus.PASS, "Guidance component verified");
			} else {
				
				test.log(LogStatus.FAIL, "Failed to verify Guidance component");
			}
		 
		 boolean status1=organisations.clickGuidanceLearnMore();
		 if(status1==true) {
				
				test.log(LogStatus.PASS, "Successfully clicked Learn more under Guidance component");
			} else {
				
				test.log(LogStatus.FAIL, "Failed to click Learn more under Guidance component");
			}
